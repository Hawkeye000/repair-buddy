...
describe "edmunds.rb" do
  ...
  describe "self#auto_style_details" do

    before do
      VCR.use_cassette 'edmunds_model_2012_aston_martin_db9_volante' do
        @model_details = Edmunds.auto_style_details(101404908, params:
            { api_key:Rails.application.secrets.edmunds_api_key })
      end
    end

    it "should return a hash with model_year_id" do
      expect(@model_details[:model_year_id]).to eq(100537350)
    end
    it "returns a hash with engine code" do
      expect(@model_details[:engine_code]).to eq("12VNAG5.9")
    end
    it "returns a hash with transmission type" do
      expect(@model_details[:transmission_type]).to eq("AUTOMATIC")
    end
